...
# Load data
data = torch.tensor(np.load("/Users/michalzmyslony/Projects/TUH/sliced_data/train/eps/slice_86211.npy.npz")["arr_0"], dtype=torch.float32)
# data = torch.tensor(np.load("/Users/michalzmyslony/Projects/TUH/sliced_data/train/no_eps/slice_10304.npy.npz")["arr_0"], dtype=torch.float32)
...
# Perform STFT
# res = torch.zeros((len(GLOBAL_DATA["labels"]), 101 * 13))
...
#     Zxx = torch.from_numpy(np.abs(Zxx).flatten())
#     res[i, :] = Zxx
    
    
x = data[0]
f, t_stft, Zxx = stft(x, fs=fs, window=w, nperseg=window_len, noverlap=window_len - shift, return_onesided=True)
